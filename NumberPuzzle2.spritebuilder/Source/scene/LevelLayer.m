...
- (void) didLoadFromCCB
{
    ...
    [m_spriteGrid addChild:sparkNode z:0 name:kTagNameSpark];
...
- (void)initLayer
{
    if (self)
    {
        ...
        CGPoint gridRelatePos = m_spriteGrid.position;
        ...
        for (unsigned char indexX = 0; indexX<MAX_ITEM_COUNT_X; indexX++)
        {
            ...
            {
                NumberItem *numberItem = (NumberItem*)[CCBReader load:@"numberItem"];//[NumberItem node];
                ...
                CGPoint pos = ccp(numberItem.contentSize.width*0.9,m_spriteGrid.contentSize.height*0.92);//ccp(numberItem.contentSize.width*0.9,winSize.height*2.97/4);
                //pos = getUiPosition(pos);//convert pos to differenet device
                pos.x = pos.x +xOffset;
                pos.y = pos.y +yOffset;
                
                
                numberItem.position = ccp(m_spriteGrid.contentSize.width/2,m_spriteGrid.contentSize.height/2);
                [m_spriteGrid addChild:numberItem z:0 name:indexString];
...
-(void)setGameLevel:(NSUInteger)level
{
    ...
    NSArray *childrenArray = [m_spriteGrid children];
...
-(void)touchEnded:(UITouch *)touch withEvent:(UIEvent *)event
{
   ...
   // NSString *currentTouchNumberString = @"";
    ...
    else
    {
        ...
        numberItem = (NumberItem*)[m_spriteGrid getChildByName:self.currentSelectIndexString recursively:NO];
        //[numberItem setItemColor:ccGRAY];
        [numberItem setItemSelect:YES];
...
-(void)itemButtonClick:(id)sender
{
    ...
    {
        NumberItem *currentNumberItem = (NumberItem*)[m_spriteGrid getChildByName:self.currentSelectIndexString recursively:NO];
...
-(void)matchRightFunctionCall
{
    ...
    NumberItem *currentNumberItem = (NumberItem*)[m_spriteGrid getChildByName:self.currentSelectIndexString recursively:NO];
    CGPoint toPosition = currentNumberItem.position;
    CCParticleSystemBase *sparkNode = (CCParticleSystemBase*)[m_spriteGrid getChildByName:kTagNameSpark recursively:NO];
    if (sparkNode)
    {
...
-(BOOL)isSectionFinish:(NSUInteger)xIndex :(NSUInteger)yIndex
{
    ...
    for (NSUInteger x = xStartIndex; x<xEndIndex; x++)
    {
        for (NSUInteger y = yStartIndex; y<yEndIndex; y++)
        {
            ...
            NumberItem *numberItem = (NumberItem*)[m_spriteGrid getChildByName:indexString recursively:NO];
